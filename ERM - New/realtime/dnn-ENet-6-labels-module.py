...
#
#     app = Flask(__name__)
#     return model, net, app


def predict_emotions(faces, frame, model):
...
def draw_frame_graphics(predictions, frame):
    process_frame = frame
...
def detect_faces(model, net):
    ...
    while True:
        ...
        predictions = predict_emotions(faces, frame, model)
        # print(predictions)

        process_frame = draw_frame_graphics(predictions, frame)
...
@app.route('/emotion')
def emotion():
    return Response(detect_faces(model, net),mimetype='multipart/x-mixed-replace; boundary=frame')
